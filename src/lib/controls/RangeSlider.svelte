<script>
  ...
  /**
   * function to run when the user scrolls on the slider while hovering
   * @param {event} e the event from browser
   **/
  ...
  // Raise lower by step if user scrolls
  function sliderWheel(e) {
    if ( !disabled && hover ) {
      const el = e.target;
      if (el === slider || slider.contains(el)) {
        const handle = index(el);
        const delta = e.deltaY > 0 ? step : -step;
        moveHandle(handle, values[handle] + delta);
        e.preventDefault();
      }
    }
  }
