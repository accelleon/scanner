...
impl DbCan {
    pub async fn all(db: &SqlitePool) -> Result<Vec<DbCan>> {
        let rows = sqlx::query!("SELECT id, name FROM cans")
            .fetch_all(db).await?;
    ...
    pub async fn query_name(db: &SqlitePool, name: &str) -> Result<DbCan> {
        let row = sqlx::query!("SELECT id, name FROM cans WHERE name = ?", name)
            .fetch_one(db).await?;
        let mut can = DbCan {
            id: row.id,
            name: row.name,
    ...
    pub async fn get(db: &SqlitePool, id: i64) -> Result<DbCan> {
        let row = sqlx::query!("SELECT id, name FROM cans WHERE id = ?", id)
            .fetch_one(db).await?;
        let mut can = DbCan {
            id: row.id,
            name: row.name,
